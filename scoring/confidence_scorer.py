...
from scoring.confidence_scorer import get_confidence
...
def extract_skills(resume_data):
    """
    Extract skills from segmented resume,
    normalize them,
    remove duplicates,
    and return structured output with confidence.
    """
    ...
    # ----------------------------------------
    # Match Against Master Skill Dictionary
    # ----------------------------------------
        ...
        for master_skill in skill_list:
            ...
            if master_skill.lower() in normalized_lower:
                ...
                extracted[category].append(
                    get_confidence(master_skill, "exact")
                )
